...
from src.base import utils_mp
from src.base import file_utils
from src.base import point_cloud
...
def convert_point_clouds(in_dir_abs, out_dir_abs, out_dir_npy_abs, target_file_type: str,
    ...
    for fi, f in enumerate(mesh_files):
        ...
        if file_utils.call_necessary(f, [file_out, file_out_npy]):
            ...
            print(" We skip the file: "+ f+" because the processed files already exists")
...
def main(dataset_name: str,num_processes=8,base_dir='datasets'):

    dataset_dir = dataset_name

    print('Processing dataset: ' + os.path.join(base_dir, dataset_dir))
    if not os.path.exists(os.path.join(base_dir, dataset_dir, '00_base_pc')):
        print('No base point clouds found in ' + os.path.join(base_dir, dataset_dir, '00_base_pc'))
        raise ValueError('No base point clouds found in ' + os.path.join(base_dir, dataset_dir, '00_base_pc'))
    ...
    convert_point_clouds(in_dir_abs=os.path.join(base_dir, dataset_dir, '00_base_pc'),
                         ...
                         out_dir_npy_abs=os.path.join(base_dir, dataset_dir, '04_pts' if only_for_evaluation else '05_query_pts'),
                         target_file_type='.xyz', target_num_points=50000, num_processes=num_processes)
    #split the dataset splits into train test and validation sets, they valdiation seems t obe split with the training set and test set will be specified in the 00_base_pointclouds
